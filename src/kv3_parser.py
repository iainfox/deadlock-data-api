...
def parse_simple_value(raw):
    """Convert a raw KV3 scalar token into a Python str/int/float.

    Quoted strings lose their quotes; numeric-looking tokens are converted
    to int or float. Anything else is returned unchanged (already a
    non-string value, or a string that doesn't match a known pattern).
    """
...
def extract_balanced(lines, start_idx):
    """Extract a balanced {...} or [...] block starting at ``lines[start_idx]``.

    Quote-awareness ensures braces/brackets inside string values don't
    affect the depth count.

    Returns (content_without_outer_braces, end_line_index).
    """
        ...
        for ch in stripped:
            ...
            if not in_string:
                if ch == open_char:
                    ...
                    if not started:
                        started = True
                        continue  # don't include the opening brace itself
                elif ch == close_char:
                    depth -= 1
                    if depth == 0:
                        if line_out:
    ...
    # Unbalanced input (shouldn't happen with well-formed KV3) - return
    # whatever we accumulated rather than raising.
    return '\n'.join(collected_lines), line_idx - 1


def parse_kv3_obj(text):
    """Parse KV3 object text (without its outer braces) into a dict."""
...
def _parse_value_on_next_line(lines, i, key, result):
    """Handle ``key =`` where the value ({...} or [...]) starts on the next line."""
    if i + 1 >= len(lines):
        return i + 1
...
def parse_kv3_arr(text):
    """Parse KV3 array text (without its outer brackets) into a list."""
    if not text or not text.strip():
        return []
...
def _parse_subclass_entry(lines, i, stripped, result):
    """Handle array entries of the form ``subclass:Foo = { ... }``."""
